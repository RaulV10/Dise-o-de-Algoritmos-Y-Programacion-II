...
int main() {
	...
	for(j = 0; j < numMaterias; j++) {
		...
		for(i = 0; i < m->numAlumnos; i++) {
			m->alumno[i].matricula = (long int*) malloc(sizeof(long int));
			m->alumno[i].nombre = (char *) malloc(20*sizeof(char));
			//m->alumno++;
		}
		...
		for(i = 0; i < m->numAlumnos; i++) {
			...
			printf("Matricula: ");
			scanf("%ld", m->alumno[i].matricula);
			
			printf("Nombre: ");
			scanf("%s", m->alumno[i].nombre);
			
			do {
				printf("Introduzca Promedio: ");
				scanf("%f", &m->alumno[i].promedio);
				if(m->alumno[i].promedio < 0 || m->alumno[i].promedio > 10) {
					printf("Error: Promedio fuera de rango");
				}
			}while(m->alumno[i].promedio < 0 || m->alumno[i].promedio > 10);
	...
	// Liberar Memoria
	for(i = 0; i < m->numAlumnos; i++) {
		free(m->alumno[i].matricula);
		free(m->alumno[i].nombre);
		//m->alumno++;
	}
