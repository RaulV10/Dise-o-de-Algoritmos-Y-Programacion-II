...
int main() {
	...
	for(j = 0; j < nr; j++) {
		...
		for(i = 0; i < r->numPel; i++){
			r->pelicula[i].nombrePel = (char *) malloc(20*sizeof(char));
			r->pelicula[i].precio = (float *) malloc(sizeof(float));
			//r->pelicula++;
		}
		...
		for(i = 0; i < r->numPel; i++) {
			printf("\n***** PELICULA #%d*****\n\n", (i+1));
			printf("Id: ");
			scanf("%ld", &r->pelicula[i].id);
			
			printf("Nombre de Pelicula: ");
		    scanf("%s", r->pelicula[i].nombrePel);
		    
			do{
				printf("Precio: $");
				scanf("%f", r->pelicula[i].precio);
				
				if(*r->pelicula[i].precio <= 0) {
					printf("El precio no puede ser negativo!\n");
				}
			}while(*r->pelicula[i].precio <= 0 );
				
			r->total += (*r->pelicula[i].precio);
	...
	// Liberar Memoria
	for(i = 0; i < r->numPel; i++) {
		free(r->pelicula[i].nombrePel);
		free(r->pelicula[i].precio);
		//r->pelicula++;
	}
